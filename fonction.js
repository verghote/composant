...
/**
 *
 * @param conteneur Balise div contenant la valeur à afficher
 * @param {int} valeurFinale : valeur finale à afficher
 */
export function afficherCompteur(conteneur, valeurFinale) {
    conteneur.innerText = '0';
    const updateCounter = () => {
        const valeurActuelle = +conteneur.innerText;
        const increment = valeurFinale / 200;

        if (valeurActuelle < valeurFinale) {
            conteneur.innerText = `${Math.ceil(valeurActuelle + increment)}`;
